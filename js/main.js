...
var utils = utils || {
  ...
  getNextElementSibling: function getNextElementSibling(el) {
  ...
  getPreviousElementSibling: function getPreviousElementSibling(el) {
  ...
  sameHeight: function sameHeight(el) {
    var maxHeight = 0;
...
var app = app || {
  ...
  init: function init() {
    utils.animationFrames();
    this.columnsHP(document.documentElement);
    this.introFade();
    this.lazyLoad();
    this.gridSameHeight();
    this.blurScrollTopLink();
  },

  columnsHP: function columnsHP(d) {
    var s = d.style;
    var column = document.querySelectorAll('.cats-hp li');
  ...
  introFade: function introFade() {
    var introText = document.getElementById('intro__inner');
    ...
    function animateIntro() {
      if (intro) {
        var scrollPercentage = (scrollPos / intro.offsetHeight).toFixed(5);
        if (window.matchMedia('(min-width: 75em)').matches && scrollPos < intro.offsetHeight) {
          introText.style.opacity = 1 - scrollPercentage * opacitySpeed;
        } else {
    ...
    window.addEventListener('resize', animateIntro);
  ...
  lazyLoad: function lazyLoad() {
    ...
    function detectLazy() {
      var _loop = function (i) {
        if (utils.elementInViewport(lazy[i])) {
          var imgAttribute = lazy[i].getAttribute('data-src');
          lazy[i].src = imgAttribute;
          lazy[i].addEventListener('load', function () {
            utils.getNextElementSibling(lazy[i]).style.display = 'none';
          });
          utils.removeClass(lazy[i], 'lazy');
        }
      };
    ...
    requestAnimationFrame(detectLazy);
    window.addEventListener('scroll', function () {
      requestAnimationFrame(detectLazy);
    });
  },

  gridSameHeight: function gridSameHeight() {
    var grid = document.querySelectorAll('.grid-posts__link');
    function resizeWindow() {
      if (window.matchMedia('(min-width: 31.25em)').matches) {
        for (var i = 0; i < grid.length; i++) {
          grid[i].style.height = 'auto';
        }
        utils.sameHeight(grid);
      } else {
        for (var i = 0; i < grid.length; i++) {
          grid[i].style.height = 'auto';
        }
  ...
  blurScrollTopLink: function blurScrollTopLink() {
    var topLink = document.querySelector('.main-footer__logo');
...
var jekyllInput = document.getElementById('search-input');
if (jekyllInput) {
  SimpleJekyllSearch.init({
    searchInput: document.getElementById('search-input'),
    resultsContainer: document.getElementById('results-container'),
    dataSource: '/articles/search.json',
    searchResultTemplate: '<li><a href="{url}" title="{desc}">{title}</a></li>',
    noResultsText: '<li>Aucun résultat trouvé</li>',
    limit: 10,
    fuzzy: false,
  })
}
